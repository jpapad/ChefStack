...
interface IngredientCostListProps {
  ...
  onAdd: () => void;
  onEdit: (cost: IngredientCost) => void;
  onDelete: (cost: IngredientCost) => void;
  canManage: boolean;
}

const IngredientCostList: React.FC<IngredientCostListProps> = ({ ingredientCosts, selectedCostId, onSelectCost, onAdd, onEdit, onDelete, canManage }) => {
       ...
       <div className="flex-1 overflow-y-auto -mr-2 pr-2">
        <div className="space-y-2">
          {ingredientCosts.map(cost => (
            ...
            >
              <div>
                <p className="font-bold text-md">{cost.name}</p>
                <p className="text-sm text-light-text-secondary dark:text-dark-text-secondary">{formatCurrency(cost.cost)} / {cost.purchaseUnit}</p>
              </div>
              {canManage && (
                <div className="flex items-center opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={(e) => { e.stopPropagation(); onEdit(cost); }} className="p-2 rounded-full hover:bg-black/10 dark:hover:bg-white/20" title="Edit">
                         <Icon name="edit" className="w-4 h-4" />
                    </button>
                    <button onClick={(e) => { e.stopPropagation(); onDelete(cost); }} className="p-2 rounded-full text-light-text-secondary hover:text-red-500 hover:bg-red-500/10" title="Delete">
...
export default IngredientCostList;
