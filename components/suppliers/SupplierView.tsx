...
const SupplierView: React.FC<SupplierViewProps> = ({ suppliers, setSuppliers, selectedSupplierId, onSelectSupplier, onBack, currentUserRole, rolePermissions }) => {
  ...
  const handleSaveSupplier = (data: Omit<Supplier, 'id' | 'teamId'> | Supplier) => {
    if ('id' in data) { // Editing
      setSuppliers(prev => prev.map(s => s.id === data.id ? data : s));
    } else { // Creating
      const newSupplier = { ...data, id: `sup${Date.now()}`, teamId: '' }; // teamId will be set in KitchenInterface
      setSuppliers(prev => [...prev, newSupplier as Supplier]);
      onSelectSupplier(newSupplier.id);
    }
    setIsFormOpen(false);
  ...
  return (
    <>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full">
         <div className={`h-full ${selectedSupplierId ? 'hidden lg:block' : 'lg:col-span-1'}`}>
            <SupplierList
         ...
         <div className={`h-full ${!selectedSupplierId ? 'hidden lg:flex' : 'lg:col-span-2'}`}>
             {selectedSupplier ? (
                <div className="p-6 h-full bg-white/60 dark:bg-slate-800/60 backdrop-blur-lg border border-white/20 dark:border-slate-700/50 rounded-2xl shadow-xl overflow-y-auto">
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        <h2 className="text-2xl font-bold font-heading">{selectedSupplier.name}</h2>
                      </div>
                       <div className="flex gap-1 flex-shrink-0 ml-4">
                        {canManage && (
                            <>
                                <button onClick={() => handleOpenForm(selectedSupplier)} title="Επεξεργασία" className="p-2 rounded-full hover:bg-black/5 dark:hover:bg-white/10 transition-colors">
                                    <Icon name="edit" className="w-5 h-5" />
                                </button>
                                <button onClick={() => handleRequestDelete(selectedSupplier)} title="Διαγραφή" className="p-2 rounded-full text-light-text-secondary hover:text-red-600 dark:text-dark-text-secondary dark:hover:text-red-400 hover:bg-red-500/10 transition-colors">
                     ...
                     <div className="mt-6 space-y-4">
                        <div className="bg-black/5 dark:bg-white/5 p-4 rounded-lg">
                            <h4 className="text-sm font-semibold font-heading text-light-text-secondary dark:text-dark-text-secondary">Υπεύθυνος Επικοινωνίας</h4>
                            <p className="text-lg font-medium">{selectedSupplier.contactPerson || '-'}</p>
                        </div>
                        <div className="bg-black/5 dark:bg-white/5 p-4 rounded-lg">
                            <h4 className="text-sm font-semibold font-heading text-light-text-secondary dark:text-dark-text-secondary">Τηλέφωνο</h4>
                            <p className="text-lg font-medium">{selectedSupplier.phone || '-'}</p>
                        </div>
                        <div className="bg-black/5 dark:bg-white/5 p-4 rounded-lg">
                            <h4 className="text-sm font-semibold font-heading text-light-text-secondary dark:text-dark-text-secondary">Email</h4>
      ...
      />
       ...
       <ConfirmationModal
        ...
        body={<p>Είστε σίγουροι ότι θέλετε να διαγράψετε τον προμηθευτή "{supplierToDelete?.name}";</p>}
      />
    </>
  );
};
